  ...
  },
  {
    ...
    verbOptions: {
      dispatch: [
        ...
        ['--name <name>', 'ticketless work; name is the request id and branch'],
        ['--slug <text>', 'branch name for a ticket; refused with --name'],
        ['--task <text>', 'instruction replacing the configured entry'],
        ['--delivery <owner>', 'child (default) or parent; parent requires --task'],
        ['--because <reason>', 'reason for assessment, override or redispatch'],
        ['--notes <file>', 'wave findings; an unreadable file refuses'],
        ['--model <selector>', 'operator choice; :effort overrides configured effort'],
        ['--capability <tier>', 'efficient, balanced or intensive'],
        ['--model-mode <mode>', 'auto, pinned or confirm; overrides project mode'],
        ['--model-confirmation <ref>', 'ask receipt: session.jsonl#toolCallId; no timeout'],
        ['--agent <name>', 'the agent Orca starts in the pane (default omp)'],
        ['--on <host>', 'a host declared in dispatch.hosts; `here` is this machine'],
        ['--repo-id <id>', 'remote repo id when --on lookup cannot answer'],
        ['--worktree <abs>', 'existing tree instead of creating one'],
        ['--needs-ref <ref>', 'required ref on origin before dispatch'],
        ['--wait <s>', 'role/model proof window; 0 skips verification'],
        ['--probe', 'throwaway placement without setup or URLs'],
        ['--dry-run', 'preview only; no worktree, record or pane created'],
