...
export function renderSpec({ job, model, issue, repo = '', draft, labels, triaged, instruction, pass = 1, previous = null, because = '' }) {
  ...
  if (job === 'triage') {
    return [
      ...
      `Leaving a group empty means you have not finished. Every name is checked twice before anything is applied — against this repository's own label list, so \`Labels: state → needs-info\` and \`Remove labels: needs-triage (superseded)\` are both refused for naming no label that exists, and against what the issue ALREADY CARRIES, so naming a provenance label it wears at birth, or asking to remove a label it does not have, is refused as a directive that would change nothing. Your directives are then graded on the RESULT they would leave: a set that would put two provenance labels on one ticket is refused whichever of them the issue already had, and a contradiction already on the issue is a tracker correction to ask for, never one to publish over.`,
      nothing,
      asking,
