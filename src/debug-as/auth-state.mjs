...
/**
 * One identity's storage state, and nothing more. A Playwright artifact is a
 * cookie list plus one or two origins' localStorage: a browser caps a single
 * cookie at 4 KiB, and a signed-in session stores a JWT pair and a handful of
 * flags — a few KiB in practice. 256 KiB admits roughly sixty maximal cookies,
 * or a session token two orders of magnitude larger than Supabase's, so no
 * honest artifact meets it; a project dump or a runaway adapter does. The file
 * is generated by project code and graded as untrusted input, so the size is
 * refused BEFORE a buffer is allocated from it, as `MAX_ADAPTER_OUTPUT` and
 * `MAX_BODY` bound the other two project-authored answers.
 */
export const MAX_STORAGE_STATE = 256 * 1024;
 ...
 */
export async function loadStorageState({ root, relativePath, browserOrigin, addresses = {}, exec = defaultExec, maxBytes = MAX_STORAGE_STATE }) {
  ...
  try {
    ...
    if (stat.size > maxBytes) {
      throw fail(
        `the authentication artifact ${inside} is ${stat.size} bytes, larger than the ${maxBytes / 1024} KiB one identity's storage state can be, so AX refused it without reading it`,
        `run the project's Debug adapter so it rewrites ${inside} as one identity's Playwright storageState, and stop whatever wrote a larger file there`,
      );
    }
