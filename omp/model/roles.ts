 ...
 *      not merely useless but wrong: `orchestrator` is an operator session role
 *      that dispatches top-level Orca children, and offering it as an in-process
 *      `task` subagent invites a spawn that cannot do the job. Keeping them out
 *      needed `task.disabledAgents`, which suppresses a symptom the mechanism
 *      itself created.
