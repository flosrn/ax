...
import { publicationMarker } from '../src/triage/publication.mjs';
...
/** The request `requestFor` mints for a job of this suite's default repository. */
const jobRequest = (issue, { job = 'triage', pass = 1, repo = 'owner/repo' } = {}) =>
  `${job}-${repo.replace(/\//g, '-')}-${issue}${pass > 1 ? `-p${pass}` : ''}`;

/** One comment the publisher would have stamped — the only shape Release accepts as this pass. */
const publication = ({ job = 'triage', repo = 'owner/repo', issue, pass = 1, createdAt, url } = {}) => ({
  body: `${publicationMarker({ job, repo, issue: String(issue), pass })}\n> *This was generated by AI during triage.*\n\nverdict\n`,
  createdAt,
  url: url ?? `https://github.com/${repo}/issues/${issue}#issuecomment-1`,
});

const commentsOf = rows => ({ status: 0, stdout: JSON.stringify({ comments: rows }), stderr: '' });
...
test('nothing is released on the report path', () => {
  ...
  const r = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 4, createdAt: '2026-08-21T09:00:00.000Z' })]) } },
  });
...
test('a dispatch is dated by the record it wrote, never by the file it lives in', () => {
  ...
  const r = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 12, createdAt: '2026-08-21T09:00:00.000Z' })]) } },
  });
  assert.match(r.out, /CLOSE.*triage pass 1 publication on #12/);
  ...
  const undated = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 13, createdAt: '2026-08-21T09:00:00.000Z' })]) } },
  });
...
test('a triage is proven by its own publication, and only that', () => {
  ...
  const after = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 7, createdAt: '2026-08-20T11:00:00.000Z' })]) } },
  });
  assert.match(after.out, /CLOSE.*triage pass 1 publication on #7/);

  const before = run(['--all'], {
    dir,
    orca: { workers: [worker('ctx_triage')], terminals: [terminal('term_ctx_triage')] },
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 7, createdAt: '2026-08-19T09:00:00.000Z' })]) } },
  });
  assert.match(before.out, /KEEP.*predates this dispatch/);
});
...
// SECOND surviving kind is proven the same way — by its own publication, never
...
test('a brief is proven by its own publication, like triage', () => {
  ...
  const after = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ job: 'brief', issue: 7, createdAt: '2026-08-20T11:00:00.000Z' })]) } },
  });
  assert.match(after.out, /CLOSE.*brief pass 1 publication on #7/);

  const before = run(['--all'], {
    dir,
    orca: { workers: [worker('ctx_brief')], terminals: [terminal('term_ctx_brief')] },
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ job: 'brief', issue: 7, createdAt: '2026-08-19T09:00:00.000Z' })]) } },
  });
  assert.match(before.out, /KEEP.*predates this dispatch/);
});
...
test('a hyphenated repository is read from the recorded identity, never guessed from the hyphens', () => {
  ...
  const r = run(['--all'], {
    ...
    execOptions: {
      repo: 'flosrn/ax-tools',
      answers: { 'gh issue view': commentsOf([publication({ repo: 'flosrn/ax-tools', issue: 7, pass: 1, createdAt: '2026-08-20T11:00:00.000Z' })]) },
    },
  });
  ...
  const r = run(['--all'], {
    ...
    execOptions: {
      answers: {
        ...
        'gh pr list': { status: 0, stdout: JSON.stringify([{ number: 91, state: 'MERGED', headRefName: 'wt' }]), stderr: '' },
      },
    },
  });
  assert.match(r.out, /KEEP.*no legal triage identity/);
  assert.doesNotMatch(r.out, /PR #91/);
  assert.ok(
    r.shell.every(line => !line.startsWith('gh pr')),
    'an illegal identity must not fall through to the parent checkout\'s pull requests',
  );
});
...
test('a landed session whose pane is still moving is BUSY, not closeable', () => {
  ...
  const r = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 8, createdAt: '2026-08-21T11:00:00.000Z' })]) } },
  });
...
test('a pane that cannot be read is never judged closed', () => {
  ...
  const { exec } = fakeExec({ answers: { 'gh issue view': commentsOf([publication({ issue: 9, createdAt: '2026-08-21T11:00:00.000Z' })]) } });
...
test('a live pane recorded here but absent from worker-list is offered, not ignored', () => {
  ...
  const r = run(['--all', '--close'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 11, createdAt: '2026-08-21T11:00:00.000Z' })]) } },
  });
...
test('two equal malformed cursors are not a quiet pane', () => {
  ...
  const r = run(['--all', '--close'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 14, createdAt: '2026-08-21T09:00:00.000Z' })]) } },
  });
...
const KEEP_FIXTURES = [
  ...
  ['a triage with no comment', keptTriage('20', { 'gh issue view': commentsOf([]) })],
  [
    'a triage whose publication predates it',
    keptTriage('21', { 'gh issue view': commentsOf([publication({ issue: 21, createdAt: '2026-08-19T10:00:00.000Z' })]) }),
  ],
  ...
  [
    'comments, none of them this pass',
    keptTriage('25', { 'gh issue view': commentsOf([{ body: 'any update?', createdAt: '2026-08-21T09:00:00.000Z' }]) }),
  ],
  [
    'another pass\'s publication',
    keptTriage('26', { 'gh issue view': commentsOf([publication({ issue: 26, pass: 2, createdAt: '2026-08-21T09:00:00.000Z' })]) }),
  ],
  [
    'an unreadable attribution',
    keptTriage('27', {
      'gh issue view': commentsOf([{ body: '<!-- ax:publication job=triage repo=owner/repo issue=27 -->\nverdict\n', createdAt: '2026-08-21T09:00:00.000Z' }]),
    }),
  ],
  [
  ...
  [
    ...
    () => {
      ...
      return run(['--all'], {
        ...
        execOptions: { answers: { 'gh pr list': prList([{ number: 91, state: 'MERGED', headRefName: 'wt' }]) } },
      });
    },
  ],
  [
  ...
  [
    ...
    () => {
      ...
      return run(['--all'], {
        ...
        execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 24, createdAt: '2026-08-21T09:00:00.000Z' })]) } },
      });
    },
  ],
  [
...
test('a comment is dated against the dispatch, not against the claim', () => {
  ...
  const r = run(['--all'], {
    ...
    execOptions: { answers: { 'gh issue view': commentsOf([publication({ issue: 15, createdAt: '2026-08-20T10:30:00.000Z' })]) } },
  });

  assert.match(r.out, /KEEP.*predates this dispatch/);
});
