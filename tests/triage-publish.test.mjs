...
test('the posted comment carries this pass\'s attribution, and a leftover draft still matches a stamped comment', () => {
  // #178: the publisher stamps the identity Release will read, AND the leftover-
  // draft gate stays the same gate — a stamped comment whose body below the
  // marker is this draft is still "this exact draft, word for word".
  const root = repo();
  draft(root, 'triage-acme-widgets-7', 'Labels: category/bug\n\nIt reproduces.\n');
  const first = run(['--issue', '7'], { root });
  assert.equal(first.code, 0);
  const posted = readFileSync(join(root, '.scratch', 'triage', 'triage-acme-widgets-7.body.md'), 'utf8');
  assert.match(posted, /<!-- ax:publication job=triage repo=acme\/widgets issue=7 pass=1 -->/);
  assert.match(posted, /This was generated by AI during triage/);

  const again = run(['--issue', '7'], { root, answers: { comments: [onIssue(posted)] } });
  assert.equal(again.code, 1);
  assert.match(again.out, /already carries this exact draft, word for word/);
  assert.deepEqual(mutations(again.calls), []);
});

test('a republish of pass 2 is associated with pass 2, never rewritten onto pass 1\'s comment', () => {
  const root = repo();
  draft(root, 'triage-acme-widgets-7', 'Labels: category/bug\n\nThe first verdict.\n');
  draft(root, 'triage-acme-widgets-7-p2', 'Labels: category/bug\n\nWhat changed since.\n');
  const r = run(['--issue', '7', '--pass', '2', '--republish'], {
    root,
    answers: { comments: [onIssue(`${AI_TRIAGE}\n\nThe first verdict.`)] },
  });
  assert.equal(r.code, 0);
  const posted = readFileSync(join(root, '.scratch', 'triage', 'triage-acme-widgets-7-p2.body.md'), 'utf8');
  assert.match(posted, /<!-- ax:publication job=triage repo=acme\/widgets issue=7 pass=2 -->/);
  assert.doesNotMatch(posted, /pass=1/);
});
