...
| `omp/model/own-record.ts` | which session file is a dispatched child's OWN, joined from ax's write-ahead record rather than from `worker-list` or the host — and why an absent record is an operator pane rather than a fault |
