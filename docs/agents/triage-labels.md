...
| source | `source:agent-found` · `source:user-report` · `source:roadmap` | already on the issue at birth — keep it, never add a second. `source:agent-found` is declared a **finding** (`triage.provenance.findings`): a triage or brief pass runs over one only when the issue names the approved obligation it serves (`Necessary for: #<spec> — <obligation>`); without that line a maintainer answers it with a verdict comment, or `to-tickets` publishes it. A triage child that meets one has been admitted by the dispatch verb — keep the birth label, never add a second. A directive set that would leave the issue carrying two of these at once is refused at publish before anything is applied, whichever of them the issue already had — and so is a draft that leaves a contradiction already on the issue standing, because the repair there is on the tracker |
