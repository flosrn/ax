...
| source | `source:agent-found` · `source:user-report` · `source:roadmap` | already on the issue at birth — keep it, never add a second. `source:agent-found` is declared a **finding** (`triage.provenance.findings`): no triage or brief pass runs over one — a maintainer answers it with a verdict comment, or `to-tickets` publishes it — so a triage child only ever meets the other two |
